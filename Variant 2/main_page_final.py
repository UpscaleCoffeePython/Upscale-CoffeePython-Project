...
def login():
    html = "<html><h1>Log In</h1>{}<br/></html>"
    ...
    logged_in_string = ["<div>Welcome, <b>{}</b>! Please proceed to the {}</div><br/>".format(i["Name"],'<a href="interface" style="vertical-align:middle"><span>Interface Page.</span></a><br/>') for i in logged_in]   
    ...
    return html.format(form) + "".join(logged_in_string)
#-------------------------------------------------------------------------------------------------
    # Attempt to process Interface Page

@app.route('/interface', methods = ['GET','POST'])
def interface():
    html = "<html><h1>Clerk Interface</h1></html>"

    logged_in = session['logged_in']
    name = ["<div><h3>Let's get to Work {}</h3></div>".format(i["Name"]) for i in logged_in]
    sales = ["<div><h3>Your Current Sales for the day is {}</h3></div>".format(i["Total Sales"]) for i in logged_in]
    commission = ["<div><h3>Your Current Commission for the day is {}</h3></div>".format(i["Total Commission"]) for i in logged_in]
    session['logged_in']=logged_in

    message = "<div><h2>Let's take more order to raise our Commission!</h2></div>"
